...
#ifdef __HAIKU__
void haiku_unregister_events(void* ctx);
int haiku_register_events(void* ctx, void (*callback)(void* ctx, const char* ifname,
	int opcode));
#endif
...
#ifdef __HAIKU__
static void
wpa_driver_haiku_event(void* global_ctx, const char* ifname, int opcode)
{
	struct bsd_driver_global* global = global_ctx;
	struct bsd_driver_data* drv;
	union wpa_event_data event;

	drv = bsd_get_drvname(global, ifname);
	switch (opcode) {
...
#if defined(__DragonFly__) || defined(__FreeBSD__) || defined(__FreeBSD_kernel__) || defined(__HAIKU__)
...
#else /* __HAIKU__ */
#ifdef HOSTAPD
#error HOSTAPD not supported yet!
#else /* HOSTAPD */
	if (haiku_register_events(global, wpa_driver_haiku_event) != 0) {
		goto fail;
	}
#endif /* HOSTAPD */
#endif /* __HAIKU__ */
...
#else
	haiku_unregister_events(global);
#endif
